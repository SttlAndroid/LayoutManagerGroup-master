...
public class EchelonLayoutManager extends RecyclerView.LayoutManager {
    ...
    public EchelonLayoutManager(Context context) {
        ...
        mItemViewWidth = (int) (getHorizontalSpace() * 0.87f);//item
        mItemViewHeight = (int) (mItemViewWidth * 1.46f);//item
    }
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler,
        ...
        mScrollOffset =
                Math.min(Math.max(mItemViewHeight, mScrollOffset + dy),
                        mItemCount * mItemViewHeight);
        layoutChild(recycler);
    ...
    private void layoutChild(RecyclerView.Recycler recycler) {
        ...
        final float offsetPercentRelativeToItemView =
        ...
        for (int i = bottomItemPosition - 1, j = 1; i >= 0; i--, j++) {
            ...
            float scaleXY =
                    (float) (Math.pow(mScale, j - 1) * (1 - offsetPercentRelativeToItemView * (1 - mScale)));
        ...
        for (int i = 0; i < layoutCount; i++) {
            ...
            layoutDecoratedWithMargins(view, left, layoutInfo.getTop(), left + mItemViewWidth,
                    layoutInfo.getTop() + mItemViewHeight);
    ...
    private void measureChildWithExactlySize(View child) {
        final int widthSpec =
                View.MeasureSpec.makeMeasureSpec(mItemViewWidth, View.MeasureSpec.EXACTLY);
        final int heightSpec =
                View.MeasureSpec.makeMeasureSpec(mItemViewHeight, View.MeasureSpec.EXACTLY);
